...
ImageFactory::~ImageFactory() {
  ...
  std::map<std::string, SDL_Surface*>::const_iterator surfIt = surfaces.begin();
 // for(auto& si : surfaces) SDL_FreeSurface(si.second);
  while(surfIt != surfaces.end()){
    SDL_FreeSurface(surfIt->second);
	 surfIt++;
  }
  
  
  std::map<std::string, SDL_Texture*>::const_iterator textIt = textures.begin();
//  for(auto& ti : textures) SDL_DestroyTexture(ti.second);
  while(textIt != textures.end()){
	  SDL_DestroyTexture(textIt->second);
	  textIt++;
  }
  std::map<std::string, Image*>::const_iterator imageIt = images.begin();
 // for(auto& fi : images  ) {
  while(imageIt != images.end()){
	 std::cout << "deleting " << imageIt->first << std::endl;
