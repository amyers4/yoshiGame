...
void HealthBar::draw() const {
  ...
  SDL_RenderDrawRect( renderer, &barRect );
	std::stringstream kingStream;
	kingStream << "Use Yoshi's tongue to capture bob-ombs";
	io.writeText(kingStream.str(), 300, 200); 
	
	std::stringstream kingStream1;
	kingStream1 << "and throw them back at the King:";
	io.writeText(kingStream1.str(), 300, 225); 
